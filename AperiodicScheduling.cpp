...
int main()
{
 ...
 int R[20], C[20], T[20] , D[20] , n ,  N ;
 float Aper_AT[20], Aper_CT[20];
...
cout << "Enter the number Aperiodic Tasks";
cin >> N ;

cout << "Enter the Tasks" ;
for(int i = 0 ; i < N ; i++)
{
  cout << "Enter the Arrival Time and Execution Time of Tasks" ;
  cin>> Aper_AT[i] >> Aper_CT[i] ;
...
// Tasks instances   // Tasks Instances Deadline
 ...
 //Display() ;
 float cur_time ;
 cur_time = Schedule(i, frame_s) ;
 // cout << "\nCurrent Time: " << cur_time ;

float slag_time = i + frame_s - cur_time ;
//cout << "\nNew slag_time: " << slag_time ;
 
 for(int a = 0 ; a < N ; a++)
 {
  if(Aper_AT[a] != -1)
  {
   if(Aper_AT[a] <= cur_time+slag_time)
   {
       if(Aper_CT[a] <= slag_time)
       {
               cout << "\nAperiodic1 Task " << a << "executes for (" << cur_time << "," << Aper_CT[a]+cur_time << ")" ;
               Aper_AT[a] = -1 ;  
               cur_time += Aper_CT[a] ;
               slag_time -= Aper_CT[a] ;
            //   cout << "\nAper_CT: " << Aper_CT[a] ; 
       ...
       {
               cout << "\n Aperiodic Task " << a << "executes for (" << cur_time << "," << Aper_CT[a]+cur_time << ")" ;
               cur_time += slag_time ;
               Aper_CT[a] -= slag_time ;
               slag_time = 0 ;
